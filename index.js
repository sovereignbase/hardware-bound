...
function concatBytes(sources) {
  if (!Array.isArray(sources)) throw new BytecodecError("CONCAT_INVALID_INPUT", "concatBytes expects an array of ByteSource items");
  if (sources.length === 0) return /* @__PURE__ */ new Uint8Array(0);
  const arrays = sources.map((source, index) => {
    try {
      return normalizeBytes(source);
    } catch (error) {
      throw new BytecodecError("CONCAT_NORMALIZE_FAILED", `concatBytes failed to normalize input at index ${index}: ${error instanceof Error ? error.message : String(error)}`);
    }
  });
...
function normalizeBytes(input) {
  ...
  if (typeof SharedArrayBuffer !== "undefined" && input instanceof SharedArrayBuffer) return new Uint8Array(input).slice();
  if (ArrayBuffer.isView(input)) {
    const view = new Uint8Array(input.buffer, input.byteOffset, input.byteLength);
    return new Uint8Array(view);
  }
  if (Array.isArray(input)) return new Uint8Array(input);
  throw new BytecodecError("BYTE_SOURCE_EXPECTED", "Expected a Uint8Array, ArrayBuffer, SharedArrayBuffer, ArrayBufferView, or number[]");
}
var ikm;
async function deriveBytes(base, domain, byteLength) {
  if (!ikm) ikm = await crypto.subtle.importKey("raw", new Uint8Array([
    0,
    1,
    2,
    3,
  ...
  return normalizeBytes(await crypto.subtle.deriveBits({
    name: "HKDF",
    hash: "SHA-256",
...
function bytesFromBase45String(base45String) {
  if (typeof base45String !== "string") throw new BytecodecError("BASE45_INPUT_EXPECTED", "bytesFromBase45String expects a string input");
  if (base45String.length % 3 === 1) throw new BytecodecError("BASE45_INVALID_LENGTH", "Base45 string length must not leave a trailing single character");
  const bytes = new Uint8Array(Math.floor(base45String.length / 3) * 2 + (base45String.length % 3 === 2 ? 1 : 0));
  let byteOffset = 0;
  for (let stringOffset = 0; stringOffset < base45String.length; ) {
    ...
    if (remaining === 2) {
      ...
      if (value2 > 255) throw new BytecodecError("BASE45_INVALID_CHUNK", `Invalid base45 chunk at index ${stringOffset}`);
    ...
    if (value > 65535) throw new BytecodecError("BASE45_INVALID_CHUNK", `Invalid base45 chunk at index ${stringOffset}`);
...
function toBase45Digit(base45String, stringOffset) {
  ...
  if (digit === -1) throw new BytecodecError("BASE45_INVALID_CHARACTER", `Invalid base45 character at index ${stringOffset}`);
  return digit;
}
...
function bytesToBase64String(bytes) {
  ...
  for (let offset = 0; offset < view.length; offset += chunkSize) {
    ...
    let chunkString = "";
    for (let index = offset; index < end; index++) chunkString += String.fromCharCode(view[index]);
    binaryString += chunkString;
  }
  if (typeof btoa !== "function") throw new BytecodecError("BASE64_ENCODER_UNAVAILABLE", "No base64 encoder available in this environment.");
  return btoa(binaryString);
}
function bytesFromBase64String(base64String) {
  if (typeof Buffer !== "undefined" && typeof Buffer.from === "function") return new Uint8Array(Buffer.from(base64String, "base64"));
  if (typeof atob !== "function") throw new BytecodecError("BASE64_DECODER_UNAVAILABLE", "No base64 decoder available in this environment.");
...
function bytesToBase64UrlString(bytes) {
  return bytesToBase64String(bytes).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
}
function base64UrlbytesToBase64String(base64UrlString) {
  ...
  else if (mod !== 0) throw new BytecodecError("BASE64URL_INVALID_LENGTH", "Invalid base64url length");
  return base64String;
}
...
async function bytesToGzipBytes(bytes) {
  ...
  if (typeof CompressionStream === "undefined") throw new BytecodecError("GZIP_COMPRESSION_UNAVAILABLE", "gzip compression not available in this environment.");
  return compressWithStream(view, "gzip");
}
async function compressWithStream(bytes, format) {
  const compressedStream = new Blob([bytes]).stream().pipeThrough(new CompressionStream(format));
...
async function bytesFromGzipBytes(bytes) {
  ...
  if (typeof DecompressionStream === "undefined") throw new BytecodecError("GZIP_DECOMPRESSION_UNAVAILABLE", "gzip decompression not available in this environment.");
  return decompressWithStream(view, "gzip");
}
async function decompressWithStream(bytes, format) {
  const decompressedStream = new Blob([bytes]).stream().pipeThrough(new DecompressionStream(format));
...
function bytesToUTF8String(bytes) {
  ...
  if (typeof Buffer !== "undefined" && typeof Buffer.from === "function") return Buffer.from(view).toString("utf8");
...
function bytesFromUTF8String(text) {
  if (typeof text !== "string") throw new BytecodecError("STRING_INPUT_EXPECTED", "bytesFromUTF8String expects a string input");
  if (typeof TextEncoder !== "undefined") {
    if (!textEncoder) textEncoder = new TextEncoder();
    return textEncoder.encode(text);
  }
  if (typeof Buffer !== "undefined" && typeof Buffer.from === "function") return new Uint8Array(Buffer.from(text, "utf8"));
...
async function createDeviceBinding(credentialName, signal) {
  ...
  try {
    ...
    if (!response || typeof response.getAuthenticatorData !== "function") return [true, "unknown"];
  ...
  try {
    ...
    if (rawId) {
      ...
      if (prf && prf?.results) {
        ...
        if (first && second) return Bytes.concat([
          rawId,
          first,
          second
        ]);
...
var credentialNameInput = document.getElementById(
  "credential-name"
);
var createButton = document.getElementById("create");
var deriveButton = document.getElementById("derive");
var status = document.getElementById("status");
var bindingExplanation = document.getElementById(
  "binding-explanation"
);
var resultOutput = document.getElementById("result");
var storageExplanations = {
  "device-bound": "The credential backing this entropy stays on this device and cannot be synced.",
  "sync-eligible": "The credential backing this entropy can be synced, but is not currently backed up.",
  synced: "The credential backing this entropy is backed up and may be available on your other devices.",
  unknown: "The browser did not reveal whether the credential backing this entropy is device-bound or synced."
};
createButton.addEventListener("click", async () => {
  ...
  status.textContent = created ? "Device binding created." : "Device binding could not be created.";
